...
import net.runelite.client.plugins.glblastfurnace.GlBlastFurnaceConfig;
import net.runelite.client.plugins.glblastfurnace.ScriptOverlay;
...
@PluginDescriptor(
        name = "Blast Furnace",
        description = "Auto blast furnace",
        tags = {"bot", "goonlite", "smithing"},
        enabledByDefault = false
)
public class GlBlastFurnacePlugin extends Plugin {
    ...
    @Inject
    private GlBlastFurnaceConfig config;

    @Provides
    GlBlastFurnaceConfig provideConfig(ConfigManager configManager)
    {
        return configManager.getConfig(GlBlastFurnaceConfig.class);
    }
    ...
    protected void startUp() throws Exception
    {
        ...
        server.start();
        String command = "";
        if (config.barTypes() == BarTypes.MITH) {
            command = "/runelite-client/src/main/resources/net/runelite/client/AutoOldSchool/smithing/bf_mith_v2.py";
        }

        else if (config.barTypes() == BarTypes.ADDY) {
            command = "/runelite-client/src/main/resources/net/runelite/client/AutoOldSchool/smithing/bf_addy_v2.py";
        }

        else if (config.barTypes() == BarTypes.Rune) {
            command = "/runelite-client/src/main/resources/net/runelite/client/AutoOldSchool/smithing/bf_rune_v2.py";
        }
    ...
    protected void shutDown() throws Exception
    {
        Process p = new ProcessBuilder(
                "/bin/sh",
                "-c",
                "pgrep -f '.*AutoOldSchool/smith*' | xargs kill -9").start();
